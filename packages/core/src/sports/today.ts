 ...
 * doesn't take the rest down with it (ARCH-R5).
 */
export async function getTodaysGames(adapters: SportsAdapter[], leagues: string[]): Promise<Game[]> {
  const targets = adapters.filter((adapter) => leagues.includes(adapter.league));
  const results = await Promise.allSettled(targets.map((adapter) => adapter.getSchedule(new Date())));
